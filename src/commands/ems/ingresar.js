                    ...
                    message.channel.awaitMessages(m => m.author.id === message.author.id, {
                        ...
                        message.channel.awaitMessages(m => m.author.id === message.author.id, {
                            ...
                            conexion.query(`INSERT INTO paciente(nombre, telefono, grupoSanguineo, fechaNacimiento) VALUES("${nombrePaciente}", "${telefonoPaciente}", "${grupoSanguineoPaciente}", "${fechaNacimientoPaciente}")`, function (error, results, fields) {
                                ...
                                message.reply('Paciente agregado.')
                                conexion.query(`SELECT * FROM paciente p WHERE nombre = "${nombrePaciente}"`, function (error, results, fields) {
                                    ...
                                    results.forEach(result => {
                                        ...
                                        const embed = new MessageEmbed()
                                            .setAuthor("Paciente: " + nombrePaciente)
                                            .addFields(
                                                {
                                                    name: 'Identificador', value: idPaciente, inline: true,
                                                    name: 'Número de teléfono', value: telefonoPaciente, inline: true,
                                                    name: 'Tipo sanguíneo', value: grupoSanguineoPaciente, inline: true,
                                                    name: 'Fecha de nacimiento', value: fechaNacimientoPaciente, inline: true
                                                }
                                            )
                                            .setColor("GREEN")
